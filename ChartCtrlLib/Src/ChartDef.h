...
public:
    ...
    PointT operator-(const PointT& pntT) const { return PointT(X - pntT.X, Y - pntT.Y); }
    PointT operator*(const PointT& pntT) const { return PointT(X * pntT.X, Y * pntT.Y); }
    PointT operator/(const PointT& pntT) const {
        ASSERT((pntT.X != 0) && (pntT.Y != 0));
...
// The circles around the data points are visible only if the minimal X-distance 
// between any two adjacent points is greater than six pixels.
// Change to a single pixel - SGS 
//#define CHART_PNTSTRSH 6.0f
#define CHART_PNTSTRSH 1.0f

// Defs for CChart container
enum CH_MODE {
    MODE_FULLX, MODE_FULLY, MODE_ZOOMINGX, MODE_ZOOMINGY,
...
#define IDM_CHART_X_AXIS        33790
#define IDM_CHART_Y_AXIS        33791
#define IDM_CHART_LEGEND_MOVE   33792
#define IDM_CHART_FIRSTCHART    33793
...
enum TUPLE_LIDX { IDX_LNAME, IDX_LNAMEX, IDX_LX, IDX_LNAMEY, IDX_LY, IDX_LCOLOR, IDX_LDASH, IDX_LPEN, IDX_LPOINT };
typedef std::tuple<string_t, string_t, string_t, string_t, string_t, Gdiplus::Color,
    Gdiplus::DashStyle, float, PointStyle> TUPLE_LABEL;
...
enum DATAVIEW_FLAGS {
    F_NODATACHANGE = 0, F_NAME = 0x0001, F_NAMEX = 0x0002, F_NAMEY = 0x0004, F_VALX = 0x0008,
...
enum STR_IDX {
    IDX_NAME, IDX_SCY, IDX_VAL, IDX_NAMEX, IDX_X, IDX_NAMEY,
    IDX_Y, IDX_COL, IDX_DSTYLE, IDX_PWIDTH, IDX_PSTYLE, IDX_STRTOTAL
};
...
typedef std::tuple<string_t, string_t, string_t, string_t, string_t, string_t, string_t,
    Gdiplus::Color, Gdiplus::DashStyle, float, PointStyle> TUPLE_PRINT;
