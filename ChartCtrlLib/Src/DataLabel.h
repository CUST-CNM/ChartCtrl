...
protected:
  ...
  // Child windows rectangles
    CRect CalcNameWndRect(CRect labStrRect, CWnd* pParent, LegendLocation legendLocation = LegendLocation::Upper_Right);
    ...
    void DrawLabelLine(RectF labelRectF, Color labCol,
        DashStyle dashStyle, float penWidth, Graphics* grPtr, PointStyle pntStyle);

public:
    bool CreateLegend(CWnd* pOwner);
    bool CreateLegend(CWnd* pOwner, const CPoint origPnt, bool bData, LegendLocation legendLocation = LegendLocation::Upper_Right);
...
protected:
    ...
    int m_fontSize = LB_FONTSIZE;           // Font size in points
  // Next members are for calculating leg wnd pos and size
    float m_fOrigOffsX = LB_STROFFSX;       // Strings offset in the data/names widows
    float m_fBorderSpace = LB_BORDERSPACE;  // Is #defined in ChartDef.h
    float m_fBulletSize = LB_BULLETSZ;      // Same
  // Layout's rectangles
    ...
    Color m_colBkgnd = Color(0xFFFFFFE0);
