...
public:
    int AddChart(bool bVisible, bool bShowPnts, string_t label, string_t labelY, int precisionY,
        ...
        Color colChart, V_CHARTDATAD& vData, PointStyle pntStyle = PointStyle::Ellipse, bool bRedraw = false);
    // Overload for time series
    int AddChart(bool bVisible, bool bShowPnts, string_t label, string_t labelY, int precisionY,
        DashStyle dashStyle, float penWidth, float tension,
        Color colChart, std::vector<double>& vTmSeries, double startX = 0.0,
        double stepX = 1.0, PointStyle pntStyle = PointStyle::Ellipse, bool bRedraw = false);
...
public:
    ...
    void ShowAxisCoordinates(bool show = true) { m_use_coordinates = show; };
protected:
...
protected:
    ...
    std::tuple <double /*minX*/, double /*maxX*/, double /*minY*/, double /*maxY*/> GetMinMaxes();
public:
    int GetScreenDpi(void);
    static void PrintCharts(CChartContainer* pContainer, int scrDpiX, HDC printDC);
    void setXAxisToLogarithmic(bool log) { m_x_axis_is_logarithmic = log; };
    void setYAxisToLogarithmic(bool log) { m_y_axis_is_logarithmic = log; };
    void setXAxisToLinear(bool lin) { m_x_axis_is_logarithmic = !lin; };
    void setYAxisToLinear(bool lin) { m_y_axis_is_logarithmic = !lin; };
    bool XAxisIsLogarithmic() const { return m_x_axis_is_logarithmic; };
...
protected:
    ...
    afx_msg void OnMnMoveLegend(void);
    afx_msg void OnMnShowAxBoundaries(void);
    afx_msg void OnMnXAxisIsLogarithmic(void);
    afx_msg void OnMnYAxisIsLogarithmic(void);
    ...
    // Data Members
protected:
    Color m_colBkground = (ARGB)Color::White;
    Color m_colBorder = (ARGB)Color::Black;
    Color m_colAxis = (ARGB)Color::Black;
    Color m_colZoomBorder = (ARGB)Color::Red;
    Color m_colLegPnts = (ARGB)Color::Red;
    Color m_colDataViewPnts = (ARGB)Color::BlueViolet;
    Color m_colLegBkgnd = Color(0xFFFFFFE0);
    Color m_colLegBorder = (ARGB)Color::Black;
    Color m_colGrid = (ARGB)Color::Gray;

    double m_minExtY = DBL_MAX;     // Min coordinate of the Y axis
    double m_maxExtY = -DBL_MAX;    // Max coordinate of the Y axis
    double m_startX = DBL_MAX;      // Start X coord
    double m_endX = -DBL_MAX;       // Last X coord (not included)
    float  m_axOffsXF = 0.0f;       // Y Offset of X axis
    AX_XPOS m_axXPos = BETWEEN;     // Position of zero on the X axis
    AX_YPOS m_axYPos = MIDDLE;      // Position of zero on the Y axis
    double m_scX = 1.0;
    double m_scY = 1.0;
    bool m_bTracking = false;       // Cross cursor and data legend are enabled
    bool m_bHasNamesLeg = false;    // Window for name legend was not created
    bool m_bUserEnabled = true;     // Enabling/Disabling the popup menu
    bool m_bAxBoundaries = false;   // Show the X axis left and right boundaries
    int  m_chartSelIdx = -1;        // ID of the chart selected to scale Y locally
    int  m_dataViewChartIdx = -1;   // ID of the chart displayed in data view
    int  m_precision = 3;           // Number of decimal digits after decimal point
                                    // for data labels
    double m_tmpZX = 0.0;           // Temporary placeholder for zoom startX
    double m_tmpZY = 0.0;           // Temporary placexolder for zoom startY

    MatrixD* m_pMatrixD = nullptr;  // Transform matrix from the data pnt space to the client coord

    PointD m_dataLegPntD = PointD(DBL_MAX, DBL_MAX);    // Transformed to the data point space point of sel line
    double m_epsX = 0.0;            // Neighbourhood to search for nearest point
...
protected:
    ...
    CDataWnd* m_pDataWnd = nullptr;
    CDataWnd* m_pLegWnd = nullptr;
    CChartDataView* m_pChartDataView = nullptr;

protected:
    CH_MODE m_chModeX = MODE_FULLX;
    CH_MODE m_chModeY = MODE_FULLY;
    string_t m_name;                        // Container's name
    string_t m_labelX = string_t(_T("X"));  // X-axis name

    val_label_str_fn m_pLabValStrFnPtr;

protected:
    bool m_use_coordinates = true;
    bool m_show_legend = true;
    bool m_first_time = true;
    bool m_x_axis_is_logarithmic = false;
    bool m_y_axis_is_logarithmic = false;
    bool m_show_y_axis_second_scale = false;
    bool m_showXLabel = false;
    bool m_showYLabel = false;
